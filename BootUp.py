...
=====================================================
Features:
  - Display enable/disable/extend/mirror
  - Per-display brightness control (WMI + gamma ramp fallback)
  - About section with full system/GPU/driver info
  - Desktop layout preview + activity log
...
import sys, os, platform, threading, time, subprocess
...
def get_all_adapters():
    ...
    while True:
        ...
        if not user32.EnumDisplayDevicesW(None, idx, byref(dev), 0):
            break
        adapters.append({
            "index":      idx,
...
def get_monitors_for(device_name):
    ...
    while True:
        mon = DISPLAY_DEVICEW()
        mon.cb = sizeof(DISPLAY_DEVICEW)
        if not user32.EnumDisplayDevicesW(device_name, idx, byref(mon), 0):
            break
        monitors.append({
...
def get_current_mode(device_name):
    ...
    if user32.EnumDisplaySettingsW(device_name, ENUM_CURRENT_SETTINGS, byref(dm)):
        return dm
    return None
...
def get_supported_modes(device_name):
    ...
    while True:
        dm = DEVMODEW()
        dm.dmSize = sizeof(DEVMODEW)
        if not user32.EnumDisplaySettingsW(device_name, idx, byref(dm)):
            break
        ...
        idx += 1
    return sorted(modes, key=lambda x: x[0]*x[1], reverse=True)
...
def enable_display(device_name, pos_x, pos_y, width, height, freq=60):
    ...
    tr = user32.ChangeDisplaySettingsExW(device_name, byref(dm), None, CDS_TEST, None)
    if tr != DISP_CHANGE_SUCCESSFUL:
        return tr
    return user32.ChangeDisplaySettingsExW(
        device_name, byref(dm), None,
        CDS_UPDATEREGISTRY | CDS_NORESET | CDS_GLOBAL, None)
...
def disable_display(device_name):
    ...
    dm.dmPelsHeight = 0
    return user32.ChangeDisplaySettingsExW(
        device_name, byref(dm), None, CDS_UPDATEREGISTRY | CDS_NORESET, None)
...
def set_brightness_powershell(level):
    try:
        cmd = (f"(Get-WmiObject -Namespace root/WMI -Class "
...
def set_brightness(device_name, level, is_primary=False):
    ...
    if is_primary:
        ...
        if set_brightness_powershell(level):
            return "PowerShell WMI", True
    ok = set_brightness_gamma(device_name, level)
    return "Gamma Ramp", ok

def get_current_brightness():
    v = get_brightness_wmi()
...
def collect_system_info():
    info = {
        "os": platform.system(), "os_ver": platform.version(),
        "os_rel": platform.release(), "machine": platform.machine(),
    ...
    for cmd, prefix in [
        ...
        try:
            ...
            for line in r.stdout.splitlines():
                if "=" in line:
                    k, v = line.split("=", 1)
                    info[f"{prefix}{k.strip().lower()}"] = v.strip()
        except Exception:
            pass
        ...
        for folder in os.listdir(driver_store):
            if "igd" in folder.lower() or "igfx" in folder.lower():
                for dll in dlls:
                    ...
                    if os.path.isfile(fp) and dll not in dll_paths:
                        dll_paths[dll] = fp
    info["dll_paths"] = dll_paths
    return info
